...
// Lossless quality levels — must stay in sync with audio-validator.js
const LOSSLESS_QUALITIES = new Set(['flac', 'flac24bit', 'master', 'hires', 'atmos']);

const SOURCE_QUALITY_HELPER_PATH = (
    fs.existsSync(path.join(__dirname, 'source-quality.js'))
        || fs.existsSync(path.join(__dirname, 'source-quality', 'index.js'))
)
    ? './source-quality'
    : null;
const SOURCE_QUALITY_HELPERS = SOURCE_QUALITY_HELPER_PATH ? require(SOURCE_QUALITY_HELPER_PATH) : {};
const {
    extractScriptInfo: extractScriptInfoFromHelper,
    analyzeSourceQuality,
} = SOURCE_QUALITY_HELPERS;
...
function parseScriptInfo(code, filename) {
    ...
    if (typeof extractScriptInfoFromHelper !== 'function') {
        return fallbackInfo;
    }

    try {
        const extracted = extractScriptInfoFromHelper(code, filename);
        ...
        return {
            ...fallbackInfo,
            ...extracted,
            name: extracted.name || fallbackInfo.name,
            description: extracted.description || fallbackInfo.description,
            version: extracted.version || fallbackInfo.version,
            author: extracted.author || fallbackInfo.author,
            homepage: extracted.homepage || extracted.repository || fallbackInfo.homepage,
            rawScript: extracted.rawScript || fallbackInfo.rawScript,
        };
    } catch (error) {
        logger.warn(`failed to extract script info: ${filename}`, {
            error: error.message,
        });
        return fallbackInfo;
    }
...
function normalizeSourceQuality(sourceQuality, context = {}) {
    const normalized = sourceQuality && typeof sourceQuality === 'object' ? sourceQuality : {};
    const score = Number(normalized.score);
    const priority = Number(normalized.priority);
    const labels = Array.isArray(normalized.labels)
        ? normalized.labels.filter(Boolean).map(item => String(item).trim()).filter(Boolean)
        : [];
    const evidence = Array.isArray(normalized.evidence)
        ? normalized.evidence.filter(item => item != null)
        : [];
    ...
    return {
        ...
        summary: typeof normalized.summary === 'string' && normalized.summary.trim()
            ...
            : (labels.join(' / ') || `未标注质量 (${filename})`),
        evidence: evidence.length ? evidence : [`source-quality unavailable for ${filename}`],
    };
}

function buildSourceQuality(filePath, code, scriptInfo) {
    const filename = path.basename(filePath);

    if (typeof analyzeSourceQuality !== 'function') {
        return normalizeSourceQuality(null, { filename });
    }

    try {
        return normalizeSourceQuality(analyzeSourceQuality({
            filePath,
            fileName: filename,
            scriptInfo,
        }), { filename });
    } catch (error) {
        logger.warn(`failed to analyze source quality: ${filename}`, {
            error: error.message,
        });
        return normalizeSourceQuality(null, { filename });
    }
...
function getSourceQualityPriority(sourceQuality) {
    const priority = Number(sourceQuality?.priority);
    if (Number.isFinite(priority)) return priority;
    return 5;
}

function getSourceQualityScore(sourceQuality) {
    const score = Number(sourceQuality?.score);
    return Number.isFinite(score) ? score : 0;
}

function hasLosslessLabel(sourceQuality) {
    const labels = sourceQuality?.labels;
    if (!Array.isArray(labels)) return false;
    return labels.includes('无损') || labels.includes('Hi-Res');
}

function comparePreparedSourcesByQuality(left, right, requestedQuality) {
    // 1. When requestedQuality is a lossless tier, sources with '无损' or 'Hi-Res' labels
    //    take priority over same-score sources without those labels.
    if (LOSSLESS_QUALITIES.has(requestedQuality)) {
        const leftLossless = hasLosslessLabel(left.executor?.sourceQuality);
        const rightLossless = hasLosslessLabel(right.executor?.sourceQuality);
        if (leftLossless !== rightLossless) {
            return leftLossless ? -1 : 1;
        }
    }

    // 2. Sort by SourceQuality.score descending
    const scoreDiff = getSourceQualityScore(right.executor?.sourceQuality)
        - getSourceQualityScore(left.executor?.sourceQuality);
    if (scoreDiff !== 0) return scoreDiff;

    // 3. Sort by SourceQuality.priority ascending (lower number = higher priority)
    const priorityDiff = getSourceQualityPriority(left.executor?.sourceQuality)
        - getSourceQualityPriority(right.executor?.sourceQuality);
    if (priorityDiff !== 0) return priorityDiff;

    // 4. Preserve original load order (alphabetical) for deterministic results
    return left.originalOrder - right.originalOrder;
}

function prepareExecutorsForDispatch({
    folderPath,
    files,
    method,
    payload,
    maxCandidates = files.length,
    prioritizeHighQualitySources = false,
}) {
    const prepared = [];

    for (const [originalOrder, file] of files.entries()) {
    ...
    if (prioritizeHighQualitySources) {
        prepared.sort((left, right) => comparePreparedSourcesByQuality(left, right, payload.quality));
    }

    const result = prepared
        .slice(0, Math.max(1, Number(maxCandidates) || prepared.length))
        .map((entry, order) => ({
            ...entry,
            order,
        }));
    ...
    for (const current of result) {
        logger.debug('prepared candidate for dispatch', {
            file: current.file,
            order: current.order,
            sourceQualityScore: current.executor?.sourceQuality?.score ?? null,
            sourceQualityPriority: current.executor?.sourceQuality?.priority ?? null,
        });
    }
...
function createLegacyExecutor(scriptInfo, source) {
    return {
        kind: 'legacy',
        scriptInfo,
        sourceQuality: scriptInfo?.sourceQuality || null,
...
function buildExecutor(filePath) {
    ...
    const filename = path.basename(filePath);
    const parsedScriptInfo = parseScriptInfo(code, filename);
    const sourceQuality = buildSourceQuality(filePath, code, parsedScriptInfo);
    const scriptInfo = {
        ...parsedScriptInfo,
        file: filename,
    ...
    const sourceConsole = createConsoleBridge(`source:${filename}`);
...
async function evaluateCandidate({ method, payload, options, folder, file, executor, order }) {
    ...
    const candidate = {
        ...
        meta: {
            ...
            sourceQuality: executor.sourceQuality || executor.scriptInfo?.sourceQuality || null,
        },
        validation: null,
    };
    ...
    if (typeof options.validateResult === 'function') {
        ...
        if (validation && validation.ok === false) {
            logger.warn(`candidate validation failed: ${file}`, {
                ...
                bitrateKbps: validation.details?.bitrateKbps ?? null,
                minBitrateKbps: validation.details?.minBitrateKbps ?? null,
                requestedQuality: payload.quality || null,
            });
            return null;
        }
    }
...
async function selectBestInFolderWithinWindow({ method, payload, options, folder, files }) {
    ...
    const prepared = method === 'url'
        ? prepareExecutorsForDispatch({
            folderPath,
            files,
            method,
            payload,
            maxCandidates,
            prioritizeHighQualitySources: true,
        })
        : (() => {
            const legacyPrepared = [];

            for (const [index, file] of files.entries()) {
                if (legacyPrepared.length >= maxCandidates) break;
...
async function smartDispatch(method, payload = {}, options = {}) {
    ...
    for (const { folder, folderPath, files } of dispatchFolders) {
        ...
        if (options.selectBestResult && options.selectionWindowMs) {
            ...
            if (bestInFolder) {
                ...
                logger.info('selected best candidate within window', {
                    ...
                    requestedQuality: payload.quality || null,
                    actualFormat: bestInFolder.validation?.details?.format || null,
                    actualBitrateKbps: bestInFolder.validation?.details?.bitrateKbps || null,
                    validationScore: bestInFolder.validation?.score || 0,
                    qualityAlignmentBonus: bestInFolder.validation?.details?.qualityAlignmentBonus || 0,
                });
        ...
        for (const current of prepared) {
            try {
                const executor = current.executor || loadExecutor(path.join(folderPath, current.file));
                if (!executor || !executor.supports(method, payload)) continue;
